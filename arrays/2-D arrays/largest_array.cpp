...
vector<int> nextSmallerElement(vector<int>& heights, int size) {
    ...
    stack<int> s;
    s.push(-1); // have pushed -1 in the stack so the while could run else
                // in the first com
...
int largestRectangleArea(vector<int>& heights) {
    ...
    for (int i = 0; i < size; i++) {
        int l = heights[i];
        if (next[i] == -1) {
            next[i] = size; // if all the heights are of the same then
                            // largest rectangle would have the breadth
                            // equal to size of the vector
        }
